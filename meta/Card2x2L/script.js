document.addEventListener('DOMContentLoaded', function() {
    ...
    function getNextSerialNumber() {
        ...
        const todayCards = cards.filter(card => 
            card.serialNumber && card.serialNumber.startsWith(dateStr));
    ...
    function renderCard(cardData, index) {
        ...
        card.innerHTML = `
            <div class="photo-container">
                ...
                <img class="photo" src="${cardData.photoUrl || ''}" alt="Inspection photo">
            ...
            <div class="card-body">
                <div class="form-group">
                    ...
                    <input type="text" id="serial-${index}" value="${cardData.serialNumber || ''}" placeholder="Auto-generated serial number">
                </div>
                <div class="form-group">
                    <label>Location</label>
                    <div id="location-${index}" class="location-data">
                        ${cardData.location ? 
                            `<span class="location-label">Lat:</span> ${cardData.location.latitude.toFixed(6)}, 
                             <span class="location-label">Long:</span> ${cardData.location.longitude.toFixed(6)}` : 
                            'Fetching location...'}
                    </div>
                </div>
                <div class="form-group">
                    <label for="comments-${index}">Comments</label>
                    <textarea id="comments-${index}" placeholder="Add your inspection notes here...">${cardData.comments || ''}</textarea>
                </div>
            </div>
            <div class="actions">
        ...
        cameraInput.addEventListener('change', function(e) {
            if (e.target.files && e.target.files[0]) {
                ...
                reader.onload = function(event) {
                    ...
                    card.querySelector('.timestamp').textContent = new Date().toLocaleString();
    ...
    function resetAllCards() {
        if (confirm('Are you sure you want to reset all cards? This will delete all current data.')) {
            cards = [];
            ...
            navigator.geolocation.getCurrentPosition(
                ...
                function(position) {
                    ...
                    locationElement.innerHTML = `
                        <span class="location-label">Lat:</span> ${location.latitude.toFixed(6)}, 
                        <span class="location-label">Long:</span> ${location.longitude.toFixed(6)}
                    `;
                },
                // Error
                function(error) {
    ...
    function exportToWord() {
        ...
        for (let i = 0; i < cards.length; i += 4) {
            ...
            for (let j = i; j < i + 4 && j < cards.length; j++) {
                ...
                const timestamp = card.timestamp ? 
                    new Date(card.timestamp).toLocaleString() : 'No timestamp';
                ...
                content += `
                    <div class="card">
                        ...
                        <div class="card-info">
                            ...
                            ${card.location ? 
                                `<p><strong>Location:</strong> ${card.location.latitude.toFixed(6)}, ${card.location.longitude.toFixed(6)}</p>` : 
                                '<p><strong>Location:</strong> N/A</p>'}
                        </div>
                        <div class="comments">
                            <p><strong>Comments:</strong> ${card.comments || 'No comments'}</p>
                        </div>
                    </div>
                `;
            }
        ...
        link.download = `Inspection_Report_${new Date().toLocaleDateString().replace(/\//g, '-')}.doc`;
        link.click();
